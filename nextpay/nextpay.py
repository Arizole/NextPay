...
class NextPayError(Exception):
    pass

class NextPay:
    def __init__(self, access_token: str = None, device_uuid: str = str(uuid.uuid4()), client_uuid: str = str(uuid.uuid4()), proxy_addr: str = None, proxy_port: int = None) -> None:
        if proxy_addr != None or proxy_port != None:
            if proxy_addr == None or proxy_port == None:
                raise NextPayError(None, "Proxyのアドレスまたはポートが設定されていません")
            self.proxy_conf = {
    ...
    def login_start(self, phone_number: str, password: str) -> None:
        ...
        if response["header"]["resultCode"] != "S0000":
            raise NextPayError(response["header"]["resultCode"], response["header"]["resultMessage"])
    ...
    def login_otp(self, otp: str) -> None:
        if not all(key in self.session.cookies for key in ["Lang", "__Secure-request_uri"]):
            raise NextPayError(None, "先にログインを開始してください")
    ...
    def get_balance(self) -> dict:
        if not "Authorization" in self.headers:
            raise NextPayError(None, "先にログインを行ってください")
    ...
    def accept_link(self, code: str, password: str = None) -> dict:
        ...
        if response["payload"]["orderStatus"] != "PENDING":
            raise NextPayError(None, "リンクは既に受け取り済みであるか辞退済みです")
        elif response["payload"]["pendingP2PInfo"]["isSetPasscode"] and password == None:
            raise NextPayError(None, "パスワードが必要です")
